...
namespace vdb
{
    ...
    HnswIndex::HnswIndex(const HnswConfig &config)
        : config_(config), rng_(config.seed), level_mult_(1.0 / std::log(static_cast<double>(config.M)))
    {
    ...
    HnswIndex::HnswIndex(HnswIndex &&other) noexcept
        : config_(std::move(other.config_)), nodes_(std::move(other.nodes_)), id_to_index_(std::move(other.id_to_index_)), entry_point_(other.entry_point_), max_level_(other.max_level_), element_count_(other.element_count_), rng_(std::move(other.rng_)), level_mult_(other.level_mult_)
    {
    }
    ...
    SearchResults HnswIndex::search_filtered(
        VectorView query,
        size_t k,
        std::function<bool(VectorId)> filter) const
    {
    ...
    Result<void> HnswIndex::resize(size_t new_max_elements)
    {
        ...
        // Cannot resize to smaller than current element count
        if (new_max_elements < element_count_)
        {
            return std::unexpected(Error{ErrorCode::InvalidInput,
                                         "Cannot resize to " + std::to_string(new_max_elements) +
                                             ", index contains " + std::to_string(element_count_) + " elements"});
        }

        // Update capacity and reserve space
    ...
    namespace
    {
        constexpr uint32_t FLAT_INDEX_MAGIC = 0x56444246; // "VDBF" - VectorDB Flat
    ...
    Result<void> FlatIndex::save(std::string_view path) const
    {
        ...
        {
            file.write(reinterpret_cast<const char *>(&ids_[i]), sizeof(VectorId));
            file.write(reinterpret_cast<const char *>(vectors_[i].data()),
                       dimension_ * sizeof(Scalar));
        }

        if (!file.good())
        {
            return std::unexpected(Error{ErrorCode::IoError, "Failed to write index data"});
        }
    ...
    Result<FlatIndex> FlatIndex::load(std::string_view path)
    {
        ...
        for (uint64_t i = 0; i < count; ++i)
        {
            ...
            file.read(reinterpret_cast<char *>(vec.data()), dimension * sizeof(Scalar));
        ...
        if (!file.good())
        {
            return std::unexpected(Error{ErrorCode::IoError, "Failed to read index data"});
        }
